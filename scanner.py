...
def scan(filename):
    ...
    image=cv2.imread(filename)   #read in the image
    ...
    pts=np.float32([[0,0],[1000,0],[1000,800],[0,800]])  #map to 800*800 target window
    
    op=cv2.getPerspectiveTransform(approx,pts)  #get the top or bird eye view effect
    scanned_img=cv2.warpPerspective(orig,op,(1000,800))
    
    # cv2.imwrite("15_scan.jpg",scanned_img)
